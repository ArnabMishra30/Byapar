...
describe('suppliers: module specific rules', () => {
  let adminToken;
  ...
  it('returns money as an exact string, not a float', async () => {
    const response = await request(app).get('/api/v1/suppliers?search=Sunrise').set(auth());
  ...
  it('preserves decimals that a float would corrupt', async () => {
    ...
    expect(response.body.data.supplier.openingBalance).toBe('0.10');
    expect(response.body.data.supplier.creditLimit).toBe('0.20');
  });
  ...
  it('defaults financial fields to zero', async () => {
    ...
    expect(response.body.data.supplier.openingBalance).toBe('0.00');
    expect(response.body.data.supplier.creditLimit).toBe('0.00');
  });
  ...
  it('searches by phone, email and GSTIN', async () => {
    const byPhone = await request(app).get('/api/v1/suppliers?search=9876543210').set(auth());
    const byEmail = await request(app).get('/api/v1/suppliers?search=sunrise.test').set(auth());
    const byGstin = await request(app).get('/api/v1/suppliers?search=29AAGCB').set(auth());
  ...
  it('accepts a party with only a name', async () => {
    ...
    expect(response.body.data.supplier.gstin).toBeNull();
  });
});
